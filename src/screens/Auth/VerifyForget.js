import { Text, TouchableOpacity, View, StyleSheet, Dimensions, ScrollView, Alert, Linking } from 'react-native'
...
export default function VerifyForget ({navigation, check}) {
      ...
      setIsLoading(true)
        if (otp === code) {
            Alert.alert(i18n.t('successVerify'), i18n.t('successVerifyText'), [
                {
                  text: 'OK',
                  onPress: () => {navigation.navigate('RestorePassword');setVisible1(false); setTimer(60); setOTP1(''); setOTP2(''); setOTP3(''); setOTP4('')},
                },
              ]);
              setIsLoading(false)
        } else {
          ...
          >
            <View style={[styles.bottomNavigationView,{backgroundColor:theme.background}]}>
              <View style={{width:"90%", marginTop:15}}>
                <Text style={{fontSize:18, color: theme.color, fontWeight:'400'}}>
                  {i18n.t('isnotverifidenumbertext')} 
                </Text>
                <TouchableOpacity 
                  style={{height:40, backgroundColor:'#2DA010', alignItems:'center', justifyContent:'center', flexDirection:"row", marginTop:40, borderRadius:10}}
                  onPress={()=> Linking.openURL(`tel:${87132766272}`)}

                >
                  <Ionicons name="md-call" size={18} color="white" />
                  <Text style={{color:'white', fontSize:16, fontWeight:"500", marginLeft:7}}>{i18n.t('call')} </Text>
                </TouchableOpacity>
...
<View style={{backgroundColor:theme.background}}>
                ...
                <TouchableOpacity activeOpacity={0.7} onPress={()=>(navigation.goBack())} style={{marginLeft: 30, marginTop:30, display: visible1 === false ? 'flex' : 'none'}}>
            <AntDesign name="closesquare" size={24} color={theme.color} style={{opacity: 0.5}} />            
          </TouchableOpacity>

          <TouchableOpacity activeOpacity={0.7} onPress={()=>(setVisible1(false), setIsSendMessage(''), setTimer(60))} style={{marginLeft: 30, marginTop:30, display: visible1 === true ? 'flex' : 'none'}}>
            <AntDesign name="leftsquare" size={24} color={theme.color} style={{opacity: 0.5}} />            
          </TouchableOpacity>
</View>
          

          <View style={{width:"100%", alignItems:'center', paddingTop:20, backgroundColor:theme.background}}>
            <Text style={{color: theme.color, fontSize:20, fontWeight:"600"}}>{visible1 === false ? i18n.t('userCheckTitle') : i18n.t('codeVerifyTitle')}</Text>
          </View>

          <View style={{flex: 1, alignItems:'center', justifyContent:'center', display: visible1 === false ? 'flex' : 'none', backgroundColor:theme.background}}>
            <View style={{flex: 1, alignItems:'center', justifyContent:'center'}}>
              ...
              >
                ...
                <Text style={{textAlign:'center', fontSize:15, marginTop:10, color:theme.color}}>
                ...
                <View style={{alignItems:'center'}}>
                  ...
                  {globalThis.verTick === true ? 
                   <View style={{flexDirection:'row', alignItems:'center', justifyContent:'center', backgroundColor:'#B0F9AA', padding:5,paddingLeft:7, paddingRight:7, borderRadius:5, marginTop:12}}>
                   <AntDesign name="checkcircle" size={14} color="#0E8204" />
                 
                     <Text style={{fontSize:14, marginLeft:5, color:'#0E8204', fontWeight:'500'}}> {i18n.t('verifidenumber')}</Text>
                   </View>
                   : 
                      <View style={{flexDirection:'row', alignItems:'center', justifyContent:'center', backgroundColor:'#FDE1BF', padding:5,paddingLeft:7, paddingRight:7, borderRadius:5, marginTop:12}}>
  <AntDesign name="exclamationcircle" size={14} color="#BE6906" />

    <Text style={{fontSize:14, marginLeft:5, color:'#BE6906', fontWeight:'500'}}>{i18n.t('notverifidenumber')}</Text>
  </View> 
                   }
                ...
                <View style={{flexDirection:'column', justifyContent:'center', alignItems:'center', marginTop:150}}>
                  ...
                  <TouchableOpacity 
                    ...
                    style={{ backgroundColor: isLoading === false ? theme.loading : '#B8B8B8', alignItems:'center', justifyContent:'center', borderRadius:10, height:40, width:200}}
                    onPress={()=>(getNumbers())}
                  >
                    {
                        isLoading === false 
                        ? <Text style={{fontSize:18, fontWeight:'600', color:theme.background}}>{i18n.t('getCodeText')}</Text>
                  ...
                  <TouchableOpacity 
                    style={{ alignItems:'center', justifyContent:'center', borderRadius:10, marginTop:15}}
                    onPress={()=>(setVisible(true),setIsWhatsNumber(false))}
                  >
                    <Text style={{fontSize:18, fontWeight:'600', color:theme.loading, textDecorationLine:'underline'}}>{i18n.t('isnotverifidenumber')}</Text>
                  </TouchableOpacity>
                </View>
          ...
          <View style={{flex: 1, alignItems:'center', justifyContent:'center', display: visible1 === true ? 'flex' : 'none', backgroundColor:theme.background}}>
            ...
            <View style={{flex: 1, alignItems:'center', justifyContent:'center'}}>
              ...
              >
                <View style={{alignItems:'center', justifyContent:'center'}}>
                  <Text style={{fontWeight:'400', color:theme.color, fontSize: 16, marginTop: 10}}>
          ...
          <View style={{ alignItems:'center', justifyContent:'center', marginTop:10}}>
            ...
            <TouchableOpacity disabled={isLoading === false ? false : true} style={{ width:260, height:50, alignItems:'center', justifyContent:'center', backgroundColor: isLoading === false ? theme.loading : '#B8B8B8', borderRadius:15}} onPress={(handleVerifyOTP)}>
              {/* <Text style={{ color:theme.background, fontSize:16, fontWeight:'600'}}>Подтвердить</Text> */}

              {
                        isLoading === false 
                        ? <Text style={{ color:theme.background, fontSize:16, fontWeight:'600'}}>{i18n.t('submit')}</Text>
          ...
          <Text style={[styles.timerText, {display: timer !== 0 ? 'flex' : 'none', color:theme.color}]}>
          ...
          {timer === 0 && (
            <TouchableOpacity style={{alignItems:'center', justifyContent:'center', marginTop: 15,}} onPress={handleResendCode}>
              <Text style={{textDecorationLine:'underline', color:theme.color}}>{i18n.t('getCodeText')}</Text>
            </TouchableOpacity>
)}
        </View>
              </ScrollView>
