import { Text, TouchableOpacity, View, StyleSheet, Dimensions, ScrollView, Alert } from 'react-native'
...
export default function LoginVerify ({navigation, check}) {
      ...
      const otp = otp1 + otp2 + otp3 + otp4;
        if (otp === code) {
            Alert.alert(i18n.t('successVerify'), i18n.t('successVerifyText'), [
                {
                  text: 'OK',
                  onPress: () => {setShowBiometric(false); setVisible1(false); setTimer(60); setOTP1(''); setOTP2(''); setOTP3(''); setOTP4('')},
                },
              ]);
        } else {
    ...
    const {iin, logout, userVerified, foreignUser, setShowBiometric} = useContext(AuthContext)
    const [respass, setresPass] = useState('')
    const [visible1, setVisible1] = useState(false);
    const [visible, setVisible] = useState(false);
    const [closed, setClosed] = useState(true);
    // globalThis.closed === clos
    ...
    // console.log(userVerified, foreignUser)
  ...
  // },[])
    ...
    return (
      <View style={{flex: 1, backgroundColor: theme.background, }}>
          ...
          <TouchableOpacity activeOpacity={0.7} onPress={()=>(logout())} style={{marginLeft: 30, marginTop:30, display: visible1 === false ? 'flex' : 'none'}}>
          ...
          <View style={{flex: 1, alignItems:'center', justifyContent:'center', display: visible1 === false ? 'flex' : 'none'}}>
            <View style={{flex: 1, alignItems:'center', justifyContent:'center'}}>
              ...
              >
                ...
                {i18n.t('whatsappTextSub')}
                </Text>
                </View>
                <View style={{alignItems:'center'}}>
                  <Text style={{fontWeight:'bold', color:theme.color, fontSize: 20, marginTop: 20}}>
                    {'+7'+respass.slice(1)}
                  </Text>
                </View>
          ...
          <View style={{flex: 1, alignItems:'center', justifyContent:'center', display: visible1 === true ? 'flex' : 'none'}}>
