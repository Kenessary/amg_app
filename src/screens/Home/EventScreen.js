...
{/* <View style={{flexDirection:'row', width: windowWidth-20, marginTop: 0,justifyContent:'space-between',backgroundColor: "white", paddingLeft: 15, paddingRight:15, paddingTop:15, borderTopLeftRadius:15, borderTopRightRadius: 15 }}>
     ...
     <View style={{ width: windowWidth-20, marginBottom:0, backgroundColor:'white', alignItems:'center', padding: 15, borderTopLeftRadius:15, borderTopRightRadius: 15}}>
 ...
 <View style={{flexDirection:'row', width: windowWidth-20, marginTop: 0,justifyContent:'flex-end',backgroundColor: "white", paddingLeft: 15, paddingRight:15, paddingBottom:15, borderBottomEndRadius: 15, borderBottomStartRadius:15 }}>
    ...
    for(let i = 0; i<uniqueYear.length; i++){
      yearArray.push(
        <TouchableOpacity key={i} style={[year === `${uniqueYear[i]}` ? styles.selectedYear : styles.year]} onPress = {() => setYear(`${uniqueYear[i]}`)}>
        <Text>{uniqueYear[i]}</Text>
      </TouchableOpacity>
      )
    }
