...
export default function EventsScreen({ navigation }) {
  ...
  let [year, setYear] = useState(new Date().getFullYear().toString());
  ...
  let event2023 = event.filter(function (value) {
    return value.data_vv.slice(0, 4) == year;
  });
  ...
  for (let i = 0; i < event2023.length; i++) {
    ...
    events.push(
      <TouchableOpacity
        ...
        onPress={() => {
          navigation.navigate("EventPdfScreen");
          setLink(event2023[i].ssilka);
        }}
      >
        ...
        >
          ...
          >
            <Text
              style={{ color: theme.color, fontSize: 15, textAlign: "left" }}
            >
        ...
        >
          <View
            style={{
              backgroundColor: theme.dateBack,
              width: 90,
              alignItems: "center",
              justifyContent: "center",
              borderRadius: 5,
        ...
        style={[
          year === `${uniqueYear[i]}`
            ? [
                styles.selectedYear,
                {
                  backgroundColor: theme.yearBack,
                  borderColor: theme.yearBorder,
                },
              ]
            : [styles.year, { backgroundColor: theme.yearBack }],
        ]}
        onPress={() => setYear(`${uniqueYear[i]}`)}
      >
  ...
  return (
    <View
      style={[
        styles.container,
        { backgroundColor: isDarkMode === true ? "#262C38" : "#F2F2F2" },
      ]}
    >
      <View
        style={{
