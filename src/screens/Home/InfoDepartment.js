  ...
  useEffect(()=>{
    ...
    axios(config)
     .then(function(response){
      ...
      // console.log(parse_third)
      let newArray = parse_third.map((list)=>{
        return {id: list.id, name: list.name, roditel: list.roditel}
      })
              ...
              <View style={styles.container1}>
                <FlatList 
                  ...
                  renderItem={({item, index})=> {
                    ...
                    return (
                      <View style={{...styles.tableRow, backgroundColor: index % 2 == 1 ? "#EFEFEF" : "white"}}>
                        <Text style={{...styles.columnRowTxt, fontWeight:"bold"}}>{item.fio}</Text>
                        <Text style={styles.columnRowTxt}>{item.doljnost}</Text>
                        <Text style={styles.columnRowTxt}>{item.raboch_tel}{"\n"}<Text style={{...styles.columnRowTxtRoom, backgroundColor: item.timein>JSON.parse(date) && item.timein >item.timeout ? "#90EE90" : "" }}>{item.cabinet}</Text></Text>
  ...
  return(
    ...
    <View>
      ...
      <View style={{flexDirection:'row', width: windowWidth-30, alignItems:'center', marginLeft: 15}}>
            <View><Text style={{fontSize: 20, color: "#4D4D4D", fontWeight:"bold", paddingVertical: 12}}>{globalThis.s}</Text></View>
      </View>
