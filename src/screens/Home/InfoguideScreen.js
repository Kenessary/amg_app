...
export default function InfoguideScreen({navigation}){
    ...
    dropdownStyle={{height: '60%', borderRadius:15}}
    defaultButtonText={i18n.t('chooseDep')}
    selectedRowStyle = {{backgroundColor: '#D64D43'}}
    selectedRowTextStyle = {{color:'white'}}
    disableAutoScroll={true}
    // search={true}
    buttonStyle={{backgroundColor:'white', width: windowWidth-20, paddingLeft:24, paddingRight:11, borderRadius:10 }}
    buttonTextStyle={{textAlign:'left', fontSize:16, color: '#4D4D4D'}}
	onSelect={(selectedItem, index) => {
		setSelected(selectedItem); navigation.navigate('InfoDepartment')
	}}
    // onFocus={(selectedItem)=>{selectedItem}}
    renderDropdownIcon={()=> <Entypo name="chevron-down" size={20} color="#4D4D4D" />}
...
/>
                    ...
                    <View style={{flexDirection:'row', alignItems:'center', width: windowWidth-20, backgroundColor:'white', height: 50, borderRadius:10, marginTop: 10}}>
                        <TextInput
                        ...
                        style={{width: 20 , height: 50, borderRadius: 10, marginLeft: 10, opacity: 0.85}}
                        ><FontAwesome5 name="search" size={13}/></Button>             
                    </View>
                    {/* <SelectList
                        data={data.map((list)=> list.descr).sort()} 
                ...
                {!shouldShow ? (
                    <View>
                    <View style={{marginLeft: 15, marginTop: 15}}><Text style={{fontSize: 14, color:"#0D6106"}}> {i18n.t('searchguideresult')} <Text style={{fontWeight: 'bold', fontSize:13,color:"#0D6106" }}>{search.length}</Text></Text></View>
                    ...
                    <View>
                        <View>
                            ...
                            return (
                                <View style={{...styles.tableRow, backgroundColor: index % 2 == 1 ? "#F0EFEF" : "white"}}>
                                <Text style={{...styles.columnRowTxt, fontWeight:"bold"}}>{item.fio}</Text>
                                <Text style={styles.columnRowTxt}>{item.doljnost}{"\n"}<Text style={styles.columnRowTxtRoom}>{item.cabinet}</Text>{"\n"}<Text style={styles.columnRowTxt}>{item.fax}</Text></Text>
                                <Text style={styles.columnRowTxt}>{item.raboch_tel}{"\n"}<Text style={styles.columnRowTxt}>{item.email}</Text></Text>
                                <Text style={styles.columnRowTxtt}>{item.sot_tel}{"\n"}<Text style={styles.columnRowTxtt}>{item.predki}</Text></Text>
                                </View>
                            )}}
                            />
                        </View>
